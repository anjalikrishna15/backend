...
loaded_rf = joblib.load("./rf.joblib")  # Loads the random forest model
...
def predict():
    ...
    if imagefile and allowed_file(imagefile.filename, ALLOWED_EXTENSIONS) and soundfile and allowed_file(soundfile.filename, ALLOWED_EXTENSIONS_SOUND):
        image_filename = werkzeug.utils.secure_filename(imagefile.filename)
        ...
        # Process the image
        img = load_img(f'./{UPLOAD_FOLDER}/{image_filename}', target_size=(224, 224))
        img = np.array(img)
        img = preprocess(img)
